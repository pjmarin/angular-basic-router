...
export class BasicRouter extends LitElement {
    ...
    async displayItems() {
        ...
        const content = fetch('https://data.cityofchicago.org/resource/8pix-ypme.json').then(async (r) => JSON.stringify(JSON.parse((await r.text())), undefined, 2));
        return content;
    }
    async displayItemsNoPretty() {
        const content = fetch('https://data.cityofchicago.org/resource/8pix-ypme.json').then(r => r.text());
        return content;
    }
    render() {
        return html `
...
], BasicRouter.prototype, "title", void 0);
